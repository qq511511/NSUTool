...
if [ -n "$(find /sdcard/ -name ".NSUKEY" -print -quit)" ]; then
   sleep 0.1
   clear
   echo -e "${NC}#!/system/bin/sh&bash || sdcard>Android"
else
clear
echo "#!/system/bin/sh&bash || sdcard>Android"
echo
echo -e "${NC}[验证] ${BOLD_CYAN}请输入脚本的Github主页链接"
echo
echo -ne "${NC}[输入] ${BOLD_GREEN}"
read git
echo "$git" > /sdcard/.NSUKEY
fi
if [ "$(cat "/sdcard/.NSUKEY")" = "https://github.com/qq511511/NSUTool" ]; then
   sleep 0.3
   echo
   echo -e "${NC}[输出] ${BOLD_CYAN}验证通过！"
else
   sleep 0.3
   echo
   echo -e "${NC}[输出] ${BOLD_RED}验证失败！"
   exit 1
fi
echo
sleep 1
